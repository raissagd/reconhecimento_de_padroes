...
h <- 0.1 * (Nall)^(-1/(8+4))  # Ajustado para 8 dimensões
...
plot(log(pxc1vec), log(pxc2vec),
     ...
     main = "Espaço das Densidades Estimadas (KDE - escala log)")
...
plot(h_values, acuracias, type = "b", pch = 19, col = "darkgreen",
     xlab = "h (largura de banda do KDE)",
     ylab = "Acurácia da Classificação",
     main = "Acurácia vs. h")
grid()
